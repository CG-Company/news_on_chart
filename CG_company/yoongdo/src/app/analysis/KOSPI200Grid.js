...
// Enhanced KOSPI200Grid Component
...
useEffect(() => {
    setLoading(true);
    fetch('http://192.168.1.105:8000/api/ticker_map')
    .then(res => res.json())
    .then(data => {
        // Add mock price data
...
return (
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
            ...
            <div>
                <h2 className="text-xl font-bold text-gray-900">KOSPI 200</h2>
                <p className="text-sm text-gray-600">대표 종목 현황</p>
            </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div key={stock.ticker || index} className="group p-4 border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200 bg-gradient-to-r from-gray-50 to-white hover:from-blue-50 cursor-pointer">
                <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${stock.trend === 'up' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    <span className="text-xs font-medium text-gray-500">#{index + 1}</span>
                </div>
                <div className={`p-1 rounded ${stock.change >= 0 ? 'bg-green-100' : 'bg-red-100'}`}>
                    {stock.change >= 0 ? (
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className={`text-sm font-medium ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {stock.change >= 0 ? '+' : ''}{stock.change?.toFixed(2) || '2.50'}%
                    </div>
                    </div>
