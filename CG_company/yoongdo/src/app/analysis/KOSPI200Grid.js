            ...
            try {
                ...
                const stockPromises = limitedTickers.map(async (ticker) => {
                    try {
                        ...
                        let keywords = [];
                        try {
                            const newsResponse = await fetch(`${API_BASE}/api/news_is_selected?ticker=${ticker.ticker}`);
                            if (newsResponse.ok) {
                                ...
                                if (newsList.length > 0) {
                                    ...
                                    if (latestNews.keyword && latestNews.keyword.trim()) {
                                        keywords = latestNews.keyword.split(',')
                                            .map(k => k.trim())
                            ...
                            console.warn(`Error fetching news for ${ticker.ticker}:`, newsError);
                        }
                                    ...
                                    <div className="flex items-start gap-2 flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap gap-1 min-h-[20px]">
                                            {stock.keywords && stock.keywords.length > 0 ? (
                                                ...
                                                <span className="text-xs text-gray-400 italic">키워드 없음</span>
                                            )}
                                        </div>
                                    </div>
