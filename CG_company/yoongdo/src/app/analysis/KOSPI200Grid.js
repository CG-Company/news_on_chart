  ...
  useEffect(() => {
    const fetchStocksWithPrices = async () => {
      ...
      try {
        ...
        // 000000 티커(거시뉴스) 제외
        const tickerList = rawTickerList.filter(
        ...
        const stockPromises = limitedTickers.map(async (ticker) => {
          try {
            ...
            const stockResponse = await fetch(
              `${API_BASE}/api/stock?ticker=${ticker.ticker}`
            );
            if (!stockResponse.ok) {
            ...
            // 최신 데이터 (배열의 마지막 요소)
            const latestStock =
              stockArray.length > 0 ? stockArray[stockArray.length - 1] : null;

            // 뉴스 키워드 가져오기
            let keywords = [];
            try {
              const newsResponse = await fetch(
                `${API_BASE}/api/news_is_selected?ticker=${ticker.ticker}`
              );
              if (newsResponse.ok) {
                const newsData = await newsResponse.json();
                const newsList = newsData.news || [];

                // 최신 뉴스의 키워드들 추출 (최대 3개)
                if (newsList.length > 0) {
                  const latestNews = newsList[newsList.length - 1]; // 가장 최근 뉴스
                  if (latestNews.keyword && latestNews.keyword.trim()) {
                    keywords = latestNews.keyword
                      .split(",")
                      .map((k) => k.trim())
                      .map((k) => k.replace(/[^\w\s가-힣]/g, "")) // 특수문자 제거 (한글, 영문, 숫자, 공백만 유지)
                      .filter((k) => k.length > 0)
                      .slice(0, 3); // 최대 3개만
                  }
                }
              ...
              console.warn(
                `Error fetching news for ${ticker.ticker}:`,
                newsError
              );
            }
            ...
            return {
              ...
              marketCap: latestStock
                ? Math.floor(
                    (latestStock.close * (Math.random() * 1000 + 100)) / 1000000
                  )
                : Math.floor(Math.random() * 50) + 10,
              trend:
                latestStock && latestStock.change_rate >= 0 ? "up" : "down",
            };
          } catch (error) {
            console.warn(
              `Error fetching stock data for ${ticker.ticker}:`,
              error
            );
