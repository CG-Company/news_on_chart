...
import { ArrowLeft, TrendingUp, Newspaper, BarChart3, Hash, Activity, Users, TrendingDown, ExternalLink, Calendar, AlertCircle } from 'lucide-react';
...
const KeywordDetailPanel = ({ keyword, onClose }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [statsResponse, newsResponse, relatedResponse] = await Promise.allSettled([
    ...
    const relatedStocks = stats?.related_tickers || keyword.tickers?.map((ticker, index) => ({
        ticker,
        name: `종목 ${ticker}`,
        sector: '기술',
    ...
    // 상관관계 차트 데이터 (모의 데이터)
    const correlationData = stats?.daily_stats?.slice(0, 7).map((stat, index) => ({
        date: new Date(stat.date).toLocaleDateString('ko-KR', { month: 'MM', day: 'dd' }),
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex items-center gap-4 text-sm text-gray-500">
                                                        <span>상관계수: {(stock.correlation || 0.75).toFixed(2)}</span>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-white rounded-lg p-6">
                                ...
                                <div className="text-sm text-gray-600 bg-white p-4 rounded-lg">
                                    💡 <strong>{keyword.keyword}</strong> 키워드는 최근 활발한 관심을 받고 있으며, 
                                    {keyword.tickers?.length || 0}개의 종목과 연관되어 있습니다. 
                                    {stats?.total_mentions || keyword.count || 0}건의 언급량을 기록했습니다.
                                </div>
                            </div>
                        </div>
                    )}
