...
const KeywordDetailPanel = ({ keyword, onClose }) => {
    ...
    const fetchTickerMap = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setTickerMap(data);
          console.log(
            "TickerMap loaded for KeywordDetailPanel:",
            data.length,
            "종목"
          );
        }
  ...
  useEffect(() => {
    const fetchRelatedStocksData = async () => {
      ...
      console.log(
        "Fetching stock prices for keyword tickers:",
        keyword.tickers
      );
      ...
      try {
        const stocksWithPrices = await Promise.all(
          keyword.tickers.map(async (tickerInfo) => {
            ...
            const ticker =
              typeof tickerInfo === "object" ? tickerInfo.ticker : tickerInfo;
            ...
            return {
              ...
              name: companyName,
              sector:
                typeof tickerInfo === "object" ? tickerInfo.sector : "기술",
              price: stockData
                ? stockData.close
                : typeof tickerInfo === "object"
                ? tickerInfo.price
                : null,
              change: stockData
                ? stockData.change_rate
                : typeof tickerInfo === "object"
                ? tickerInfo.change_rate
                : null,
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (newsResponse.status === "fulfilled" && newsResponse.value.ok) {
          const newsReturnData = await newsResponse.value.json();
          setNews(newsReturnData.news || []);
        } else {
          console.warn("NewsReturn API failed:", newsResponse.statusText);
          setNews([]);
        }
  ...
  const correlationData = (() => {
    if (stats?.daily_stats && stats.daily_stats.length > 0) {
      ...
      return [
        {
          date: "07-01",
          mentions: keyword.count || 45,
          priceChange: 1.2,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-02",
          mentions: Math.floor((keyword.count || 45) * 1.5),
          priceChange: 2.8,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-03",
          mentions: Math.floor((keyword.count || 45) * 0.5),
          priceChange: -0.5,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-04",
          mentions: Math.floor((keyword.count || 45) * 2),
          priceChange: 3.1,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-05",
          mentions: Math.floor((keyword.count || 45) * 3.5),
          priceChange: 4.7,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-08",
          mentions: Math.floor((keyword.count || 45) * 2.2),
          priceChange: 2.3,
          tickerCount: keyword.tickers?.length || 2,
        },
        {
          date: "07-09",
          mentions: Math.floor((keyword.count || 45) * 3),
          priceChange: 3.9,
          tickerCount: keyword.tickers?.length || 2,
        },
      ];
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-white rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                    <div className="text-2xl font-bold text-green-600">
                      {relatedStocksWithPrices.length ||
                        keyword.tickers?.length ||
                        0}
                    </div>
                  ...
                  <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                    <div className="text-2xl font-bold text-purple-600">
                      {stats?.avg_daily_mentions?.toFixed(1) ||
                        ((keyword.count || 0) / 7).toFixed(1)}
                    </div>
                  ...
                  <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                    <div
                      className={`text-2xl font-bold ${
                        relatedStocksWithPrices.length > 0
                          ? relatedStocksWithPrices.reduce(
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">
                        {
                          relatedStocksWithPrices.filter(
                            (stock) => stock.hasRealData
                          ).length
                        }{" "}
                        / {relatedStocksWithPrices.length} 종목
                      </span>
                      <div className="flex items-center gap-1">
                ...
                <div className="text-sm text-gray-600 bg-white p-4 rounded-lg">
                  💡 <strong>{keyword.keyword}</strong> 키워드는 최근{" "}
                  {stats?.period || "30일"}간 활발한 관심을 받고 있으며,
                  {relatedStocksWithPrices.length ||
                    keyword.tickers?.length ||
                    0}
                  개의 종목과 연관되어 있습니다. 총{" "}
                  {stats?.total_mentions || keyword.count || 0}건의 언급량을
                  기록했으며, 관련 종목들의 평균 변동률은{" "}
                  <strong>
                    {relatedStocksWithPrices.length > 0
                      ? `${
                          relatedStocksWithPrices.reduce(
                  ...
                  입니다.
                  {relatedStocksWithPrices.filter((stock) => stock.hasRealData)
                    .length > 0 &&
                    ` (${
                      relatedStocksWithPrices.filter(
                        (stock) => stock.hasRealData
