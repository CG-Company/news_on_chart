...
export default function NewsPage() {
  ...
  const handleOpenFeedModal = () => {
    ...
    const deadlineStr = `${plus30.getFullYear()}.${String(plus30.getMonth() + 1).padStart(2, '0')}.${String(plus30.getDate()).padStart(2, '0')} 까지`;
    setFeedForm((prev) => ({
      ...prev,
      name: tickerName,
      code: ticker,
      ...
      <div className="flex-1 ml-52">
        ...
        <main className="p-8 bg-gray-50 min-h-screen overflow-x-auto">
          ...
          {showFeedModal && (
            ...
            >
              <form className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative" onSubmit={handleFeedSubmit}>
                {/* X 버튼 */}
                <button
                  type="button"
                  ...
                  aria-label="닫기"
                >
                  <img src="/icons/exit_white.svg" alt="닫기" className="w-8 h-8 transition duration-150 hover:brightness-110" />
                </button>
                <h3 className="text-lg font-bold mb-4">피드 작성</h3>
                <div className="mb-2">
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>종목명</label>
                  <input
                    ...
                    disabled
                  />
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>종목코드</label>
                  <input
                    className="w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 text-gray-400 bg-gray-100"
                    placeholder="종목코드"
                    value={feedForm.code}
                    disabled
                  />
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>현재가</label>
                  <input
                    className="w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 text-gray-400 bg-gray-100"
                    placeholder="현재가"
                    value={feedForm.price ? Number(feedForm.price).toLocaleString() : ""}
                    disabled
                  />
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>등락률</label>
                  <input
                    className="w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 text-gray-400 bg-gray-100"
                    ...
                    disabled
                  />
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>투자 의견</label>
                  <select
                    ref={opinionRef}
                    className={`w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 appearance-none focus:border-blue-600 focus:border-2 focus:outline-none ${opinionError ? 'border-red-500 border-2' : ''}`}
                    style={{
                      background: "url('/icons/arrow-down.svg') no-repeat right 1rem center/1.2em auto",
                      paddingRight: "2.5rem"
                    }}
                  ...
                  >
                    <option value="" disabled hidden style={{ color: '#b0b0b0' }}>
                      목록에서 항목을 선택하세요
                    </option>
                  ...
                  {opinionError && isSubmitted && (
                    <div className="relative flex items-center gap-2 text-white text-xs mb-2 bg-red-500 rounded-xl px-3 py-2 shadow-lg" style={{ width: 'fit-content' }}>
                      <img src="/icons/warning.svg" alt="경고" className="w-4 h-4" />
                      투자 의견을 선택해 주세요.
                      <span
                        className="absolute left-4 -top-2 w-3 h-3"
                        style={{ background: '#ef4444', clipPath: 'polygon(50% 0, 0 100%, 100% 100%)' }}
                      />
                    </div>
                  )}
                  <label className="block mb-1 text-sm flex items-center">
                    ...
                    >
                      <svg width="12" height="12" fill="none" viewBox="0 0 12 12">
                        ...
                        <text x="6" y="9" textAnchor="middle" fontSize="8" fontWeight="bold" fill="#fff">!</text>
                      </svg>
                      {showTargetTooltip && (
                  ...
                  <input
                    ref={targetRef}
                    className={`w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 focus:border-blue-600 focus:border-2 focus:outline-none ${targetError ? 'border-red-500 border-2' : ''}`}
                  ...
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>요약/설명</label>
                  <textarea
                    ref={summaryRef}
                    className={`w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 focus:border-blue-600 focus:border-2 focus:outline-none ${summaryError ? 'border-red-500 border-2' : ''}`}
                    placeholder="요약/설명"
                  ...
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>작성자</label>
                  <input
                    ref={authorRef}
                    className={`w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 focus:border-blue-600 focus:border-2 focus:outline-none ${authorError ? 'border-red-500 border-2' : ''}`}
                    placeholder="작성자"
                  ...
                  <label className="block mb-1 text-sm"><span className="text-red-500 mr-1">*</span>마감일</label>
                  <input
                    ref={deadlineRef}
                    className={`w-full rounded-xl px-3 py-2 mb-2 text-sm border border-gray-300 text-gray-400 bg-gray-100 focus:border-blue-600 focus:border-2 focus:outline-none ${deadlineError ? 'border-red-500 border-2' : ''}`}
