...
function CleanChartContainer({
  ...
  const chartData = useMemo(() => {
    ...
    const markerPoints = labels
      .map((date, i) =>
        keywordMarkerDates.includes(date) ? { x: date, y: lineData[i] } : null
      )
      .filter(Boolean);
    return {
      labels,
  ...
  function externalTooltipHandler(context) {
    ...
    if (tooltipLocked && lockedTooltipData) {
      ...
      const mainCompanyNews =
        (lockedTooltipData.companyNews && lockedTooltipData.companyNews[0]) ||
        null;
    ...
    if (
      ...
      const candleData = aggregatedData
        ...
        .map((d, idx, arr) => {
          ...
          const change_rate =
            prevClose !== 0 ? ((d.close - prevClose) / prevClose) * 100 : 0;
          return {
  ...
  // === 뉴스 요약 SSE 연결 useEffect (summaryPeriod 등 상태를 위에서 선언 후 사용) ===
  useEffect(() => {
    let cancelled = false;
    ...
    const url = `${baseUrl}/api/news_summary/stream?ticker=${ticker}&period=${summaryPeriod}`;
    ...
    try {
      ...
      setSummaryError("EventSource 생성 실패: " + err.message);
      setSummaryLoading(false);
      return;
    }

    eventSource.onopen = (e) => {
      console.log("✅ SSE 연결 성공:", url, e);
    };
    ...
    eventSource.onmessage = (event) => {
      ...
      if (!event.data || event.data.trim() === "") {
        // heartbeat 등 빈 데이터는 무시
        return;
      }
      try {
        const data = JSON.parse(event.data);
        if (data.error) {
          setSummaryError(`요약 오류: ${data.error}`);
          setSummaryLoading(false);
          eventSource.close();
          return;
        }
        if (data.content) {
          ...
          if (typeof data.newsCount === "number") newsCount = data.newsCount;
          if (typeof data.cached === "boolean") cached = data.cached;
        }
      } catch (err) {
        // 빈 데이터가 아닌데도 파싱 오류가 발생하면만 에러 처리
        console.error("❌ JSON 파싱 오류:", err, { data: event.data });
        setSummaryError("요약 데이터를 처리하는 중 오류가 발생했습니다.");
        setSummaryLoading(false);
        eventSource.close();
      }
    };
    ...
    eventSource.onerror = (err) => {
      ...
      setSummaryError("서버와의 연결에 실패했습니다.");
      setSummaryLoading(false);
      eventSource.close();
    };
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            value={keywordMarker}
            onChange={(e) =>
              setKeywordMarker && setKeywordMarker(e.target.value)
            }
          ...
          {keywordMarker && (
            <button
              onClick={() => setKeywordMarker && setKeywordMarker("")}
