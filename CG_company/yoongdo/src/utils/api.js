...
export async function checkApiHealth() {
  try {
    ...
    // 먼저 health 엔드포인트 시도
    try {
      const healthResponse = await fetchWithTimeout(`${API_BASE}/api/health`, {}, 3000);
    ...
    try {
      const testResponse = await fetchWithTimeout(`${API_BASE}/api/ticker_map`, {}, 5000);
      const isHealthy = testResponse.ok;
      console.log(`${isHealthy ? '✅' : '❌'} API 서버 상태: ${isHealthy ? '정상' : '비정상'} (실제 API 테스트)`);
      return isHealthy;
    } catch (testError) {
